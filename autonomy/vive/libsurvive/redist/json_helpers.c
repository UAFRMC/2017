...
void check_asprintf(int asprintf_ret) {
  if (asprintf_ret<0) perror("asprintf allocation failure");
}

#ifdef _MSC_VER
#include <stdarg.h>

// Windows doesn't provide check_asprintf, so we need to make it.
...
void json_write_float_array(FILE* f, const char* tag, float* v, uint8_t count) {
	...
	char * str1 = NULL;
	char * str2 = NULL;
	check_asprintf(asprintf(&str1,"\"%s\":[", tag));

	for (i=0;i<count;++i) {
		if ( (i+1) < count) {
			check_asprintf(asprintf(&str2, "%s\"%f\"", str1,v[i]));
		} else {
			check_asprintf(asprintf(&str2, "%s\"%f\",", str1,v[i]));
		}
		free(str1);
	...
	check_asprintf(asprintf(&str2, "%s]", str1));
	fputs(str2,f);
	free(str1);
	free(str2);
}
...
char* load_file_to_mem(const char* path) {
	...
	if (1!=fread( JSON_STRING, len, 1, f )) perror("fread");
	fclose( f );
	return JSON_STRING;
}
