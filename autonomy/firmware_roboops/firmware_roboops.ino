 ...
 * For Arduino Uno, hooked to Robo-Ops hardware.
 */
...
void send_motors(void) {
  sendMotor(0,128-robot.power.left);
  low_latency_ops();
  sendMotor(1,128-robot.power.right);
  low_latency_ops();
  
  relay_control(robot.power.mine,2);
  
  relay_control(robot.power.dump,4);
  relay_control(128-robot.power.dump,5);
}

void relay_control(unsigned char power,int pin)
{
  digitalWrite(pin,(power>80)?LOW:HIGH);
}
