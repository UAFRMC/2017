...
// #define binIProject(v) binX*(v).x + binY*(v).y + binZ*(v).z
	...
	//bool to deal with resetting last error between different kinect steps
	...
	if(useKinect){
		...
		if(initRun){ lastErr=err; initRun=false; }
		...
		initRun=true;	//Kinect run is over, need to reinitalize
	}
	...
	x=15.0f*foot; glVertex2f(x,0.0f); glVertex2f(x,armsPower);
	x=16.0f*foot; glVertex2f(x,0.0f); glVertex2f(x,conveyorPower);
	glEnd();
...
int kinectPixelWatcher::classify_pixel(int x,int y,debug_t &debug) const
{
	...
	const float normal_Y_max=0.7; // surface normal Y component (above here is floor)
	...
	const int delx=15/decimate,dely=10/decimate; // pixel shifts for neighbor search	
	...
	debug.r=m*256; // Range, in meters
	debug.b=toFloor*2.0*256; // Up, in half-meters
	...
	if (N.y>normal_Y_max) return 4; // that's the floor (pointing upward)
