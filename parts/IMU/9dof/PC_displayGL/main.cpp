...
public:
	...
	vec3 compass_raw; // last compass reading
	vec3 compass; // field strength, in Gauss
	...
	// Print 3D vector in reasonable format
	void print(const char *name,const vec3 &v) {
		printf("%s	%+5.3f	%+5.3f	%+5.3f	",name,v.x,v.y,v.z);
	}
	...
	// Compass
		compass=compass_raw*(2.56/1000.0);
	...
	void process_serial(const std::string &line) {
		...
		if (format!="A") std::cerr<<"Unrecognized line format '"<<line<<"'\n";
			...
			// accel_raw+=vec3(-40,0,0); // fix bias (sadly, bias is time-dependent)
			...
			if (format!="G") std::cerr<<"Unrecognized line format at G (gyro)\n";
				...
				if (format!="C") std::cerr<<"Unrecognized line format at C (compass)\n";
					...
					compass_raw=read_xyz(ss);
					compass_raw=vec3(compass_raw.z,compass_raw.x,compass_raw.y);
					compass_raw+=vec3(0,0,0); // fix bias?
				}
			}
		}
	}
	...
	// Compass vector:
		...
		glVertex3fv(origin);
		vec3 comp=origin+world_from_local(compass_raw)*(0.3/100);
		glVertex3fv(comp);
