...
void accumXYZ(const union XYZBuffer *s,struct XYZAccum *d) {
  d->x+=s->value.x;
  d->y+=s->value.y;
  d->z+=s->value.z;
}
...
void setupAccel(int device) {
  ...
  Wire.endTransmission();
  Wire.beginTransmission(device);
  Wire.write(0x38); // FIFO_CTL register
  Wire.write(0x84); // stream mode, FIFO holds 4 samples
  Wire.endTransmission();
...
// at http://dlnmh9ip6v2uc.cloudfront.net/datasheets/Sensors/Magneto/HMC5883L-FDS.pdf
  ...
  Wire.write(0x70);  // average 8 samples per reading, output at 15Hz
  Wire.endTransmission();
  Wire.beginTransmission(device);
  Wire.write(0x01);   // Config register B
  Wire.write(0xA0);   // set gain to 2.56 milligauss / LSB
  Wire.endTransmission();
  Wire.beginTransmission(device);
...
enum {n=10};
void output(struct XYZAccum *a) {
  Serial.print(a->x/n);
  Serial.print('	');
  Serial.print(a->y/n);
  Serial.print('	');
  Serial.print(a->z/n);
  a->x=a->y=a->z=0; // clear accumulated values
}
